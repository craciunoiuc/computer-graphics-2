...
void Laborator2::Init()
{
	...
	no_of_generated_points = 3;	//number of points on a Bezier curve
	...
	control_p1 = glm::vec3(-4 + static_cast<float>(rand() % 10) / 5.f, -2.5 + static_cast<float>(rand() % 10) / 5.f, 1.0);
	control_p2 = glm::vec3(-2.5 - static_cast<float>(rand() % 10) / 5.f, 1.5 + static_cast<float>(rand() % 10) / 5.f, 1.0);
	control_p3 = glm::vec3(-1.5 - static_cast<float>(rand() % 10) / 5.f, 3.0 + static_cast<float>(rand() % 10) / 5.f, 1.0);
	control_p4 = glm::vec3(-4.0 - static_cast<float>(rand() % 10) / 5.f, 5.5 + static_cast<float>(rand() % 10) / 5.f, 1.0);
...
void Laborator2::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_2 && no_of_instances > 1) {
		no_of_instances--;
	}
	if (key == GLFW_KEY_3) {
		no_of_generated_points++;
	}
	if (key == GLFW_KEY_4 && no_of_generated_points > 3) {
		no_of_generated_points--;
	}
};
